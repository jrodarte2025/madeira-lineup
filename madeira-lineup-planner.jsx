...
// =============================================
function FieldPosition({ pos, player, isHighlighted, onDragStart, onDragEnd, onDragOver, onDrop, onClick, onDoubleClick }) {
  const has = !!player;
  return (
    <div draggable={has} onDragStart={has ? onDragStart : undefined} onDragEnd={onDragEnd}
      ...
      <div style={{
        width: has ? 46 : 34, height: has ? 46 : 34, borderRadius: "50%",
        ...
        fontFamily: fontDisplay, fontSize: has ? 17 : 9, fontWeight: 800,
      ...
      <div style={{
        fontSize: has ? 10 : 9, fontWeight: 700, color: has ? C.white : "rgba(255,255,255,0.35)",
        textAlign: "center", textShadow: "0 1px 4px rgba(0,0,0,0.9)",
        maxWidth: 76, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: fontBase,
      }}>{has ? player.name.split(" ").slice(-1)[0] : pos.label}</div>
      {has && <div style={{ fontSize: 8, fontWeight: 700, color: C.orange, letterSpacing: "1.2px", textShadow: "0 1px 3px rgba(0,0,0,0.9)", opacity: 0.85 }}>{pos.label}</div>}
    </div>
  );
}
...
// SAVE / LOAD MODAL
// =============================================
function SaveLoadModal({ isOpen, mode, savedLineups, onSave, onLoad, onDelete, onClose }) {
  ...
  return (
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        background: C.navyLight, borderRadius: 14, padding: 24, width: 360, maxHeight: "80vh",
  ...
  const toggleInactive = (playerId) => {
    setInactiveIds((prev) => {
      if (prev.includes(playerId)) {
        ...
        // Remove from both lineups if they're assigned
        setLineups((prevL) => ({
  ...
  return (
    <>
      ...
      <div className="screen-view" style={{
        ...
        {/* HEADER */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px", borderBottom: `1px solid ${C.whiteAlpha}`, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{
              width: 40, height: 46, background: `linear-gradient(145deg, ${C.orange}, ${C.orangeLight})`,
              clipPath: "polygon(50% 0%, 100% 14%, 100% 72%, 50% 100%, 0% 72%, 0% 14%)",
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 15, fontWeight: 900, color: C.navy, fontFamily: fontDisplay,
            }}>M</div>
            <div>
              <h1 style={{ fontFamily: fontDisplay, fontSize: 21, fontWeight: 800, letterSpacing: "-0.3px", lineHeight: 1.1 }}>MADEIRA FC</h1>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: "2.5px", fontWeight: 700, textTransform: "uppercase", marginTop: 1 }}>Lineup Planner</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: 4 }}>
              {[1, 2].map((h) => (
            ...
            <div style={{ display: "flex", gap: 4 }}>
              ...
              <button onClick={() => { setModalMode("load"); setModalOpen(true); }} style={{
                ...
                Load
                {savedLineups.length > 0 && (
                  <span style={{ background: C.orange, borderRadius: 10, padding: "1px 5px", fontSize: 9, fontWeight: 800, color: C.white, marginLeft: 2 }}>
                    {savedLineups.length}
                  </span>
                )}
              </button>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          {/* ROSTER SIDEBAR */}
          <div style={{ width: 230, borderRight: `1px solid ${C.whiteAlpha}`, display: "flex", flexDirection: "column", background: "rgba(0,0,0,0.12)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, overflow: "auto" }}>
            <div style={{
              ...
              background: pitchBg, borderRadius: 14, overflow: "hidden",
              ...
              <div style={{
                ...
                borderRadius: 20, fontSize: 10, fontWeight: 700, fontFamily: fontDisplay,
        ...
        {/* BENCH BAR */}
        <div style={{ padding: "9px 24px", borderTop: `1px solid ${C.whiteAlpha}`, display: "flex", alignItems: "center", gap: 14, background: "rgba(0,0,0,0.12)" }}>
            ...
            {availablePlayers.length === 0 ? (
              ...
              availablePlayers.map((p) => (
                <div key={p.id} draggable onDragStart={(e) => handleDragStart(e, p.id, "roster")} onDragEnd={handleDragEnd}
                  ...
                  style={{
                    display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 20, cursor: "grab",
      ...
      {/* SAVE/LOAD MODAL */}
      <SaveLoadModal isOpen={modalOpen} mode={modalMode} savedLineups={savedLineups}
